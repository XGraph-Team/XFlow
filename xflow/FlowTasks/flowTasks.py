...
def simulate_np(g, s, beta, gamma, diameter):
    ...
    for _ in range(diameter):
        ...
        node_states = snapshot['node_states'].copy()
        snapshots.append(snapshot)
...
def simulate_torch(g, s, beta, gamma, diameter):
    ...
    for _ in range(diameter):
        ...
        g = snapshot['graph']
        snapshot['graph'] = from_networkx(g.copy())
        snapshots.append(snapshot)
...
def run_sim(distance, interval_lower, obs_storage, g, s, beta, gamma, diameter):
...
# Generates pairs of random graph observations, with the lower time interval observation first (forwards) or second (backwards).
# 
# - Distance - distance between graph observations.
...
def forward(distance, interval_lower = -1, num_results=10, obs_storage = 'numpy', graph_size=1000, graph_beta = 0.1, infection_beta = None, gamma = None):
